...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $stats = [
            ...
            'offer_earned' => (float) OfferRedemption::whereIn('offer_id', $partner->offers()->pluck('id'))->where('status', 'used')->whereHas('offer', fn($q) => $q->where('price_xp', '>', 0))->sum('partner_earnings'),
            'offer_commission' => (float) OfferRedemption::whereIn('offer_id', $partner->offers()->pluck('id'))->where('status', 'used')->whereHas('offer', fn($q) => $q->where('price_xp', '>', 0))->sum('admin_commission'),
